...
export const createProduct = async (req, res) => {
  try {
    ...
    if (user.isVerified) {
      ...
      let likedUsers = await Promise.all(
        likedUserEmails.map((likedMail) => User.findOne({ email: likedMail }))
      );
      likedUsers.map((likedUserName) =>
        LikeEmail(
          likedUserName.email,
      ...
      return res
        .status(201)
        .json({ message: `New Product Created: ${productName}` });
    } else {
...
export const getProduct = async (req, res) => {
  try {
    ...
    const products = await Product.findById({ userId: id })
      .sort({ createdAt: -1 })
      .lean()
      .exec();
...
export const addComment = async (req, res) => {
  try {
    ...
    if (!comment || comment.length === 0) {
      return res
        .status(400)
        .json({ error: "Comment comment cannot be empty." });
    }
